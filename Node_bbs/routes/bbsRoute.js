...
이 내장클래스도 이슈가 있어서 실제로 코딩에서 DB와 연동하여 사용할떄
문쪠뜰을 일으킨따
...
//bbsModel에 선언된 schema를 가져외서 bbsVO 모델 생성
var bbsVO = require("../models/bbsModel");

// localhost:3000/bbs/list URL 접근했을떄
router.get("/list", function (req, res) {
  // bbsVO Model 을 통해서 데이터를 모두 읽어오고(find(조건))
...
router.get("/write", function (req, res) {
  ...
  // insert를 수행할때 비어잇는 vo를 만들어서 bbsWrite에 전달해줘야 한다.
  let data = new bbsVO();
  res.render("bbsWrite", { bbsVO: data });
});

// form에 데이터를 입력하고 전송버튼을 클릭했을떄 호출되는 URL
...
router.post("/write", function (req, res) {
  ...
  // 각각의 변수에 값을 세팅하라
  req.body.b_date = moment(new Date()).format("YYY-MM-DD"); // 대문자 월 MM
  req.body.b_time = moment(new Date()).format("HH:mm:ss"); // 소문자 분 mm
  req.body.b_count = 0;

  // form에 전송받은 데이터를 통째로 bbsVO 객체로 생성하라
  ...
  data
    ...
    .then(function (bbsVO) {
      // insert가 성공하면
      //res.json(bbsVO); // client 에게 다시 데이터를 보여라
      // 완료가 되면 리스트 화면으로 점프
...
// localhost:3000/bbs/view/id 값 URL 요청
...
router.get("/view/:id", function (req, res) {
  ...
  bbsVO
    ...
    .then(function (result) {
      ...
      //res.json(result);
      // bbsView.pug를 rendering 할떄 bbsVO라는 이름으로 전달하여 주어라
...
조건문 없이 remove() 를 실행하면 테이블의 모든데이터를 묻지도 따지지도 않고 삭제해버린다.
...
두가지 함수를 사용할 수 있다.
deleteOne(조건문) : 삭제 한 후 삭제되었다는 메시지만 알고 싶을때
fidOneAndDelete(조건문) : 삭제를 수행한 후 실제 삭제된 데이터를
  다시 확인하고 싶을때
...
router.get("/update/:id", function (req, res) {
  let id = req.params.id;
  bbsVO
    .findOne({ _id: id }) // 앞에 _id값은 bbsVO의 값이고 뒤 에 ID는 라우터 id값이다
    .then(function (result) {
...
req.body에는 id값이 사라진 사앹로 전달된다.
하지만 update 를 수행할대 id값을 URL에 전달하여 보냈기 때문에
